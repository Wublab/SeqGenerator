...
from diffuseq.esm2.constants import proteinseq_toks
...
class Alphabet(object):
    ...
    def get_single_converter(self, truncation_seq_length: int = None):
        return SingleConverter(self, truncation_seq_length)
...
class SingleConverter(object):
    """Callable to convert a string to tokens.
    """
    ...
    def __call__(self, raw_seq: str):
        # RoBERTa uses an eos token, while ESM-1 does not.
        seq_encoded = self.alphabet.encode(raw_seq)
        seq_encoded = [self.alphabet.cls_idx] + seq_encoded + [self.alphabet.eos_idx]
...
class BatchConverter(object):
    ...
    def __call__(self, raw_batch: Sequence[Tuple[str, str]]):
        ...
        labels = []
        strs = []
        print("seq_encoded_list",seq_encoded_list)
        ...
        print("seq_encoded_list22",tokens)
