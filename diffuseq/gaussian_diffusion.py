...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, t, token=None, clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        """
        ...
        model_output = model(token, x, self._scale_timesteps(t), **model_kwargs)
        ...
        if self.predict_xstart:
            pred_xstart = process_xstart(model_output['mid'])
        else:
            ### model is used to predict eps
            pred_xstart = process_xstart(
                self._predict_xstart_from_eps(x_t=x, t=t, eps=model_output['mid'])
            )
    ...
    def training_losses_seq2seq(self, model, t, model_kwargs=None, noise=None):
        ...
        model_output = model(input_ids_x, x_t, self._scale_timesteps(t))
    
        assert model_output['mid'].shape == target.shape == x_start.shape
        terms["mse"] = mean_flat((target - model_output['mid']) ** 2)


        model_out_x_start = self._x0_helper(model_output['mid'], x_t, t)['pred_xstart'] # predicted_xstart = model_output
        t0_mask = (t == 0)
...
class _WrappedModel:
    ...
    def __call__(self, token, x, ts, **kwargs):
        map_tensor = th.tensor(self.timestep_map, device=ts.device, dtype=ts.dtype)
        new_ts = map_tensor[ts]
        if self.rescale_timesteps:
            new_ts = new_ts.float() * (1000.0 / self.original_num_steps)
        return self.model(token, x, new_ts)
